...
> Source unique — fusion de `NOTES2.md` + `Notes.md` (todo.md n'existait pas). Mise à jour : 2026-08-24. `[x]` = fait, `[ ]` = reste à faire.

## UI & expérience

- [x] **Vue grille kanban switchable** (bouton « Vue libre / Vue colonnes ») : colonnes libres (créer/renommer/dupliquer/supprimer/déplacer via dropdown), drag+snap col/row, règle gauche→droite (liens vers colonnes strictement supérieures), colonnes générées automatiquement pour les pipelines existants, cartes auto-size au contenu (packing par hauteurs mesurées), colonne 0 non supprimable — *aussi listé dans NOTES2 : colonnes/rows, drag+snap, règle gauche→droite, colonnes générées auto, cartes auto-size, colonne 0 fixe, liens routés par couloirs*
...
- [x] Undo/redo : 2 piles (50 max), commit par geste (dragStop), raccourcis Ctrl+Z/Shift+Z/Y, snapshots avec colonnes — *NOTES2 : Revenir en arrière/en avant (2 piles de 50)*
...
- [x] Première lettre des catégories en majuscule — *issu de NOTES2*
- [ ] Animation « convoyeur de camions » sur les liens entre blocs
- [ ] Sélecteur multi-runs dans le panel Résultats
- [ ] Responsive mobile de l'éditeur / projets / auth (desktop-first assumé) — *NOTES2 : Mobile first*
- [ ] Ajouter des animations avec animejs
- [ ]  
...
- [x] **Gestion d'erreur GPU** : job vide `{}` corrigé (racine : `expire_on_commit` de SQLAlchemy → `session.refresh(job)` avant retour), erreur affichée + polling arrêté sur 4xx (hors 429) — *NOTES2 : Gestion d'erreur par le GPU*
...
- [ ] Template de modèles pour que les gens partent d'une base à modifier — *issu de NOTES2*

## Divers

*Anciens fichiers `NOTES2.md` et `Notes.md` fusionnés ici. `NOTES2.md` peut être supprimé — ce fichier est désormais la source unique.*

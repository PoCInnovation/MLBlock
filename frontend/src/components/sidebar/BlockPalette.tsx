...
export default function BlockPalette({ startPaletteDrag }: BlockPaletteProps) {
  ...
  const cat   = catalog.categories.find(c => c.id === selectedCat)
  const types = Object.keys(catalog.blocks).filter(t => catalog.blocks[t].cat === selectedCat)
  ...
  return (
    <div style={{ width: 256, flexShrink: 0, background: '#221c19', borderRight: '1px solid rgba(255,255,255,.06)', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
      <div style={{ padding: '16px 18px 12px', display: 'flex', alignItems: 'center', gap: 9, borderBottom: '1px solid rgba(255,255,255,.05)', flexShrink: 0 }}>
        <span style={{ width: 12, height: 12, borderRadius: 4, background: colorFor(selectedCat, catalog.categories), display: 'inline-block' }} />

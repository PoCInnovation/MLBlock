...
def test_columns_absent_from_response(client: TestClient):
    """La vue grille est retirée : aucune clé `columns` dans la réponse pipeline."""
    nodes = [
    ...
    created = client.post(
        "/api/pipelines",
        json={"name": "cols", "nodes": nodes, "edges": []},
    ).json()
    assert "columns" not in created
    fetched = client.get(f"/api/pipelines/{created['id']}").json()
    assert "columns" not in fetched
